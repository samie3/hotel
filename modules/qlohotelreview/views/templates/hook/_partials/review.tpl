        ...
        <div class="col-sm-6">
            {if $logged && !$review.response_helpful}
                <a href="#" class="btn-helpful" data-id-hotel-review="{$review.id_hotel_review}">
                    <span>{if $lang_iso == 'ar'}هل تجد هذا مفيداً؟{else}{l s='Do you find this helpful?' mod='qlohotelreview'}{/if}</span>
                    <i class="icon icon-thumbs-o-up text-primary"></i>
                </a>
            {/if}
            <p class="helpful-count"><span>{$review.total_useful|escape:'html':'UTF-8'}</span>{if $lang_iso == 'ar'} شخص وجد هذا مفيداً{else}{l s=' people found it helpful.' mod='qlohotelreview'}{/if}</p>
        </div>
        {if $logged && !$review.response_report}
            <div class="col-sm-6 text-right">
                <a href="#" class="btn-report-abuse" data-id-hotel-review="{$review.id_hotel_review}">
                    <span>{if $lang_iso == 'ar'}الإبلاغ عن إساءة{else}{l s='Report abuse' mod='qlohotelreview'}{/if}</span>
                </a>
            </div>
        {/if}
    </div>
                ...
                <div class="col-sm-offset-1 col-sm-11">
                    <div class="reply-box">
                        <div class="reply-header">
                            ...
                            <p class="heading-secondary">{if $lang_iso == 'ar'}رد على{else}{l s='has replied on' mod='qlohotelreview'}{/if} {$review.reply_date|date_format}</p>
                        </div>
