								...
								<div class="row">
									...
									{if !isset($restricted_country_mode) && !$PS_CATALOG_MODE && !$order_date_restrict}
										<p class="rm_left col-sm-12 col-md-5" {if !empty($display_all_room_types) || $room_v['room_left'] > $warning_num} style="display:none"{/if}>
											{if $lang_iso == 'ar'}<span class="remain_rm_qty">{$room_v['room_left']|escape:'htmlall':'UTF-8'}</span> {l s='بقية الغرف'}{else}{l s='Hurry!'} <span class="remain_rm_qty">{$room_v['room_left']|escape:'htmlall':'UTF-8'}</span> {l s='rooms left'}{/if}
										</p>
									{/if}
								</div>
							{/block}
										...
										<div class="col-sm-12 col-md-7 col-lg-6">
											{if !isset($restricted_country_mode) && !$PS_CATALOG_MODE && !$order_date_restrict  && (!isset($display_all_room_types) || !$display_all_room_types)}
												<p class="rm_price_cont">
													...
													{/if}
													<span class="rm_price_txt">{if $lang_iso == 'ar'}/ليلة{else}/{l s='Per Night'}{/if}</span>
												</p>
											{/if}
										</div>
									{/block}
									...
									<div class="col-sm-12 col-md-6 col-lg-8">
										{block name='room_type_list_room_booking_fields'}
											{if !isset($restricted_country_mode) && !$PS_CATALOG_MODE && !$order_date_restrict}
												...
												{else}
					{block name="room_type_list_room_book_now_no_dates"}
						<div class="rm_book_now_no_dates">
							<a href="{$room_v["product_link"]|escape:"htmlall":"UTF-8"}" class="btn btn-default button button-medium"><span>{l s="Book Now"}</span></a>
						</div>
					{/block}
													{block name='room_type_list_room_price'}
