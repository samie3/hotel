...
class WkHotelTestimonialData extends ObjectModel
{
    ...
    public function getTestimonialData($active = 2, $idLang = false)
    {
        ...
        $sql = 'SELECT tm.`id_testimonial_block`, tm.`active`, tm.`position`, tm.`date_add`, tm.`date_upd`,
                       tml.`name`, tml.`designation`, tml.`testimonial_content`
                FROM `'._DB_PREFIX_.'htl_testimonials_block_data` tm
    ...
    public function insertModuleDemoData()
    {
        ...
        $htlTestimonialContent = array(
            ...
            'ar' => 'إليك بعض التعليقات القيّمة من ضيوفنا.',
        );
