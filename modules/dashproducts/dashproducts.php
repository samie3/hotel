...
class DashProducts extends Module
{
	...
	public function getTableMostViewed($date_from, $date_to, $id_hotel)
	{
		...
		if (Configuration::get('PS_STATSDATA_PAGESVIEWS')) {
			...
			$body = '<div class="alert alert-info text-left">'.$this->l('You must enable the "Save global page views" option from ').'<u><a href="'.$link.'" target="_blank">Data mining for statistics</a></u>'.$this->l(' module in order to display the most viewed room types, or use the YajesApps Google Analytics module.').'</div>';
		}
		return array('header' => $header, 'body' => $body);
	}
